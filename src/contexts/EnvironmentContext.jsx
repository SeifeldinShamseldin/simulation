...
export const EnvironmentProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const handleCaptureRequest = () => {
      ...
      // Capture environment objects
      loadedObjects.forEach(obj => {
        if (obj.category === 'human') {
          // Handle humans separately
          const human = spawnedHumans.find(h => h.id === obj.instanceId);
          if (human) {
            environmentData.humans.push({
              id: human.id,
              name: human.name,
              isActive: human.isActive,
              position: humanPositions[human.id] || { x: 0, y: 0, z: 0 }
            });
          }
        } else {
          // Regular environment objects
          const sceneObj = sceneSetupRef.current?.environmentObjects?.get(obj.instanceId);
          if (sceneObj) {
            environmentData.objects.push({
              id: obj.instanceId,
              ...
              path: obj.path,
              position: {
                x: sceneObj.position.x,
                y: sceneObj.position.y,
                z: sceneObj.position.z
              },
              rotation: {
                x: sceneObj.rotation.x,
                y: sceneObj.rotation.y,
                z: sceneObj.rotation.z
              },
              scale: {
                x: sceneObj.scale.x,
                y: sceneObj.scale.y,
              ...
              visible: sceneObj.visible !== false
            });
          }
        }
      });
      
      EventBus.emit('world:environment-captured', environmentData);
    };
    
    // Handle world restore request
    const handleRestoreRequest = async (environmentData) => {
      console.log('[EnvironmentContext] Restoring environment from world data...');
      
      // Restore objects
      if (environmentData.objects) {
        for (const objData of environmentData.objects) {
          try {
            await loadObject({
              id: objData.objectId,
              name: objData.name,
            ...
            console.error(`[EnvironmentContext] Failed to restore object ${objData.id}:`, error);
          }
        }
      }
      
      // Restore humans
      if (environmentData.humans) {
        for (const humanData of environmentData.humans) {
          try {
            await loadObject({
              id: 'human',
              name: humanData.name,
            ...
            // Update position and active state
            setTimeout(() => {
              const humanId = spawnedHumans[spawnedHumans.length - 1]?.id;
              if (humanId) {
                updateObject(humanId, { position: humanData.position });
                
                if (humanData.isActive) {
                  handleMoveHuman(humanId);
                }
              }
            }, 100);
            
          } catch (error) {
            console.error(`[EnvironmentContext] Failed to restore human ${humanData.id}:`, error);
          }
        }
      }
    };
    ...
    const handleClearRequest = () => {
      console.log('[EnvironmentContext] Clearing all environment objects...');
      clearAllObjects();
    };
    
    const unsubscribeCapture = EventBus.on('world:capture-environment', handleCaptureRequest);
    const unsubscribeRestore = EventBus.on('world:restore-environment', handleRestoreRequest);
    const unsubscribeClear = EventBus.on('world:clear-requested', handleClearRequest);
  ...
  }, [loadedObjects, spawnedHumans, humanPositions, loadObject, updateObject, 
      handleMoveHuman, clearAllObjects, sceneSetupRef]);
