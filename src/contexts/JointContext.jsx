  ...
  export const JointProvider = ({ children }) => {
    ...
    const receiveJoints = useCallback((robotId, values) => {
      ...
      if (success) {
        ...
        // Get fresh joint values
        const allJointValues = getRobotJointValues(robotId);
        // NOTIFY EVERYONE! 🎉
        // Notify IK
        if (ikCallbackRef.current) {
          ikCallbackRef.current(robotId, allJointValues);
        }
        // Notify Trajectory (THIS IS WHAT YOU'RE MISSING!)
        if (trajCallbackRef.current) {
          debugJoint(`📹 Notifying trajectory recorder with joints:`, allJointValues);
          trajCallbackRef.current(robotId, allJointValues);
        }
        // Emit events for UI
        EventBus.emit('robot:joints-changed', {
          robotId,
    ...
    // Refactored: setJointValue now ensures up-to-date values
    const setJointValue = useCallback((robotId, jointName, value) => {
      ...
      if (success) {
        ...
        const currentJoints = getRobotJointValues(robotId);
        // MANUALLY UPDATE the value we just set to ensure it's current
        const updatedJoints = {
          ...currentJoints,
          [jointName]: value  // Force the new value
        };
        debugJoint(`🎯 Manual joint update: ${jointName} = ${value}`);
        debugJoint(`📊 All joints:`, updatedJoints);
        // Now receiveJoints has the GUARANTEED up-to-date values
        return receiveJoints(robotId, updatedJoints);
      }
      return false;
    }, [findRobotWithFallbacks, getRobotJointValues, ensureJointAngleSync, receiveJoints]);
    ...
    const getJointValues = useCallback((robotId) => {
      // ALWAYS get directly from robot for most up-to-date values
