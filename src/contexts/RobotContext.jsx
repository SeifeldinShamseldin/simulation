...
useEffect(() => {
  ...
  const handleCaptureRequest = () => {
    ...
    loadedRobots.forEach((robotData, robotId) => {
      ...
      robotsData.push({
        ...
        name: robotData.name,
        urdfPath: robotData.urdfPath,
        isActive: robotData.isActive || activeRobotId === robotId,
        position: container ? {
    ...
    EventBus.emit('world:robots-captured', robotsData);
  };
  
  // Handle world restore request
  const handleRestoreRequest = async (robotsData) => {
    console.log('[RobotContext] Restoring robots from world data...');
    
    for (const robotData of robotsData) {
      try {
        // First add to workspace if not already there
        const workspaceRobot = workspaceRobots.find(r => 
          r.robotId === robotData.id || r.urdfPath === robotData.urdfPath
        );
        
        if (!workspaceRobot) {
          // Find in available robots
          const availableRobot = availableRobots.find(r => 
            r.id === robotData.id || r.urdfPath === robotData.urdfPath
          );
          
          if (availableRobot) {
            addRobotToWorkspace(availableRobot);
          }
        }
        
        // Load robot
        await loadRobot(robotData.id, robotData.urdfPath, {
          position: robotData.position,
          makeActive: robotData.isActive,
          clearOthers: false
        });
        
        // Set joint values after a small delay to ensure robot is ready
        setTimeout(() => {
          if (robotData.jointValues && Object.keys(robotData.jointValues).length > 0) {
            setJointValues(robotData.id, robotData.jointValues);
          }
        }, 100);
        
      } catch (error) {
        console.error(`[RobotContext] Failed to restore robot ${robotData.id}:`, error);
      }
    }
  };
  ...
  const handleClearRequest = () => {
    console.log('[RobotContext] Clearing all robots...');
    
    // Unload all robots
    loadedRobots.forEach((_, robotId) => {
      unloadRobot(robotId);
    });
  ...
  const unsubscribeCapture = EventBus.on('world:capture-robots', handleCaptureRequest);
  const unsubscribeRestore = EventBus.on('world:restore-robots', handleRestoreRequest);
...
}, [loadedRobots, workspaceRobots, availableRobots, activeRobotId, getJointValues, 
    addRobotToWorkspace, loadRobot, setJointValues, unloadRobot, setActiveRobotId]);
